package com.netappsid.jira.reports;
...
public class CreationReport extends AbstractReport
{
	...
	public String generateReportHtml(ProjectActionSupport action, Map params) throws Exception
	{
		...
		if (maxCount > 0)
		{
			openIssuesCounts.forEach(issueCount -> issueBarHeights.add((issueCount.floatValue() / maxCount) * MAX_HEIGHT));
		}
...
/*
 * The code retrieves the parameters specified by the user. Then it gets the relevant issue counts from the configured time range; they are divided into the
 * time intervals specified by the user. The issue counts are normalized, giving the user a balanced histogram. Finally, the relevant details are passed to the
 * Velocity template.
 */